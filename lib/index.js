...
function getDataset (dir, dataset) {
  if (dataset) {
    ...
    debug('EXTRACT DATASET: %s/%s -> %s', dir, dataset, process.env.DATA_PATH)
...
function runStandalone (experiment, logHandler) {
  var standalone = {
    ...
    binds: [
      ...
      `${process.env.WORKSPACES_PATH}/${experiment._id}:/workspace`
    ]
  }
...
function getResults (experiment) {
  const results_file = path.join(process.env.WORKSPACES_PATH, experiment._id, 'overall.json')
  return new Promise(function (resolve, reject) {
    try {
